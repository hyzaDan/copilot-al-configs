...
Use this prompt as the VS Code launcher for the installed `snapshot-debugger` agent from this plugin.
...
Execution intent:
- adopt the `snapshot-debugger` analysis contract
...
If the environment cannot hand off automatically to the named agent, follow the same analysis contract manually and keep the output aligned with the `snapshot-debugger` agent definition.
