...
Use this prompt as the VS Code launcher for the installed `develop-orchestrator` agent from this plugin.
...
Execution intent:
- adopt the `develop-orchestrator` workflow contract
...
If the environment cannot hand off automatically to the named agent, follow the same orchestration contract manually and keep the output aligned with the `develop-orchestrator` agent definition.
